...
そのため運用面では、マイクロコード／BIOS／プラットフォーム・ファームウェアの継続的更新と、それに伴う TCB の再評価・必要に応じた再アテステーションが必須です。
...
さらに OP Stack の Superchain Interoperability は「1 ブロック遅延かつ信頼最小のクロスチェーン合成」を目指しており、Unichain はこれと接続することで**“ロールアップ横断の流動性アクセスのホーム”**になることを掲げています。
...
- Credible Neutrality
  バリデータがロールアップのメンプールを継続的に観測し、first-seen からの取り込み遅延を測ります。特定アカウントだけ取引が遅れる、優先度順序が恣意的に崩される、といった偏りを事実ベースで可視化できるようになります。
- Limiting Posting
...
#### TEE Block Builder：プライバシと表現力の拡張

- 暗号化 mempool
  ユーザーが暗号化したトランザクションを TEE に預け、実行直前まで平文を露出しない形を目指します。サンドイッチ攻撃など、事前観測に起因する bot による抜け駆けを抑え、プレトレード・プライバシを高める狙いです。
- スケジュール済みトランザクション
